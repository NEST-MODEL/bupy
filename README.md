...
8. (Опционально, для ИИ в Bupy Help) Добавьте переменную `VITE_GEMINI_API_KEY` в GitHub Actions Variables — см. `docs/GEMINI_SETUP.md`.

## Bupy Help с бесплатным ИИ

Статический справочник первой помощи работает всегда, без всего дополнительного. Ответы на свободные вопросы и распознавание паспорта через фото работают через бесплатный Gemini API — по умолчанию через общий ключ проекта (переменная `VITE_GEMINI_API_KEY` в GitHub Actions), пользователь при желании может подключить свой личный ключ в Профиле вместо общего. Подробности и важные компромиссы такого выбора — **`docs/GEMINI_SETUP.md`**.
