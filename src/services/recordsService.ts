...
export async function listVaccinations(uid: string, petId: string): Promise<Vaccination[]> {
  const snap = await withTimeout(getDocs(query(subcol(uid, petId, 'vaccinations'), orderBy('date', 'desc'))));
...
export async function addVaccination(uid: string, petId: string, input: VaccinationInput) {
  const now = Date.now();
  await withTimeout(addDoc(subcol(uid, petId, 'vaccinations'), { ...input, createdAt: now, updatedAt: now }));
}
export async function updateVaccination(uid: string, petId: string, id: string, input: Partial<VaccinationInput>) {
  await withTimeout(updateDoc(doc(subcol(uid, petId, 'vaccinations'), id), { ...input, updatedAt: Date.now() }));
}
export async function deleteVaccination(uid: string, petId: string, id: string) {
  await withTimeout(deleteDoc(doc(subcol(uid, petId, 'vaccinations'), id)));
}
...
export async function listMedications(uid: string, petId: string): Promise<Medication[]> {
  const snap = await withTimeout(getDocs(query(subcol(uid, petId, 'medications'), orderBy('startDate', 'desc'))));
...
export async function addMedication(uid: string, petId: string, input: MedicationInput) {
  const now = Date.now();
  await withTimeout(addDoc(subcol(uid, petId, 'medications'), { ...input, createdAt: now, updatedAt: now }));
}
export async function deleteMedication(uid: string, petId: string, id: string) {
  await withTimeout(deleteDoc(doc(subcol(uid, petId, 'medications'), id)));
}

export async function listMedicationLogsForDate(uid: string, petId: string, date: string): Promise<MedicationLog[]> {
  const snap = await withTimeout(getDocs(query(subcol(uid, petId, 'medicationLogs'), where('date', '==', date))));
...
export async function setMedicationLog(
  ...
  await withTimeout(setDoc(doc(subcol(uid, petId, 'medicationLogs'), id), { medicationId, date, time, status, at: Date.now() }));
}
export async function clearMedicationLog(uid: string, petId: string, medicationId: string, date: string, time: string) {
  const id = `${medicationId}_${date}_${time}`;
  await withTimeout(deleteDoc(doc(subcol(uid, petId, 'medicationLogs'), id)));
}
...
export async function listEvents(uid: string, petId: string): Promise<CalendarEvent[]> {
  const snap = await withTimeout(getDocs(query(subcol(uid, petId, 'events'), orderBy('date', 'asc'))));
...
export async function addEvent(uid: string, petId: string, input: EventInput) {
  const now = Date.now();
  await withTimeout(addDoc(subcol(uid, petId, 'events'), { ...input, createdAt: now, updatedAt: now }));
}
export async function deleteEvent(uid: string, petId: string, id: string) {
  await withTimeout(deleteDoc(doc(subcol(uid, petId, 'events'), id)));
}
